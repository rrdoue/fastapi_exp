...
This project uses PostgreSQL as its host database server.  That is, the dependencies and environmental variable file have been set up for PostgreSQL.  We're assuming that most of us are not database administrators and could use some suggestions about hosting the hr_sample database on PostgreSQL.  What may prove more difficult than installing PostgreSQL is configuring it for access.  Installation on any modern operating system shouldn't be too difficult and GUI PostgreSQL clients also exist, making it easier to manipulate data for those of us with less database command syntax experience.  However, configuring PostgreSQL access can be confusing.  Some of the following paragraphs attempt to highlight and explain problems that we've encountered over time in no particular order.

## Installation

We're not that proficient at searching the internet, but for most operating systems, Enterprise DB is one recommendation for the installer if you aren't on linux with a standard offering.  We are running a network of older MacOS systems, and started using PostgreSQL after a MacOS-only vendor, eggerapps.at, had released server and client offerings that are more than adequate for our requirements.  We're running PostgreSQL version 14, way behind the latest version available, but more than enough for our simple database.  Our database host is an older Macintosh running that server build and our other systems are running their GUI client, Postico. 

## Configuration

No matter what os one is running, accessing the server may require some configuration changes to the pg_hba.conf file after installation.  The exception may occur when one is running both the FastAPI instance and the PostgreSQL server on the same host, since the default PostgreSQL installation includes lines for localhost access.  The lines in the pg_hba.conf file that support local access include lines 89, 91, and 93.  These lines allow access for any user that has a user id on the local system and trust that user without any required password.  However, those same local users must have access to the sample database, and this depends on who installed it.
...
In order for changes to the pg_hba.conf file to take effect (except for Windows, where the changes are in service immediately), one must either reload the configuration or restart the server.  More expert personnel on DBA Stack Exchange have warned about the reload process failing to update the file, but we have not experienced this problem.  Reloading the configuration varies with operating system, so check internet sources for the best alternative.  For example, linux systems may have some shorthand syntax.

Your pg_hba.conf changes may not work the first time you try them.  In order to avoid getting too upset, just ensure that you back up the previous working version.  Better yet, call the first backup file pg_hba.conf.dist.  If you're really concerned, you can include a date and a number or time for all changes occurring on that date, like the following:
...
I tend to execute the reload from a terminal command line, but not everyone is comfortable with the terminal.  As an example, the full reload command takes the form of the following:
...
Internet searches should provide numerous helpful suggestions about every topic in this file.